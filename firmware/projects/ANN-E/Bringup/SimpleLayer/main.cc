...
int main() {
    ...
    for (int i = 0; i < bindings::kNumNeuronsPerLayer; i++) {
        ...
        bindings::bias_pots[i].SetTerminalConnections(bias_tcona_settings[i],
                                                      bias_tconb_settings[i],
                                                      bias_tconw_settings[i]);
        bindings::fb_pots[i].SetPosition(feedback_pot_positions[i]);
        bindings::fb_pots[i].SetTerminalConnections(feedback_tcona_settings[i],
    ...
    for (int i = 0; i < bindings::kNumNeuronsPerLayer; i++) {
        for (int j = 0; j < bindings::kNumWeightsPerNeuron; j++) {
            ...
            std::cout << "Neuron " << i << " Weight " << j
                      << " Position: " << std::to_string(pot_position)
                      << std::endl;
        }
        pot_position = bindings::bias_pots[i].GetPosition();
        std::cout << "Neuron " << i
                  << " Bias Position: " << std::to_string(pot_position)
                  << std::endl;
        bindings::bias_pots[i].GetTerminalConnections(&tcon_a, &tcon_b,
                                                      &tcon_w);
        std::cout << "Neuron " << i
                  << " Bias Terminal Connections: " << std::to_string(tcon_a)
                  << " " << std::to_string(tcon_b) << " "
                  << std::to_string(tcon_w) << std::endl;

        pot_position = bindings::fb_pots[i].GetPosition();
        std::cout << "Neuron " << i
                  << " Feedback Position: " << std::to_string(pot_position)
                  << std::endl;
        bindings::fb_pots[i].GetTerminalConnections(&tcon_a, &tcon_b, &tcon_w);
        std::cout << "Neuron " << i << " Feedback Terminal Connections: "
                  << std::to_string(tcon_a) << " " << std::to_string(tcon_b)
                  << " " << std::to_string(tcon_w) << std::endl;
    }
