...
int main() {
    ...
    bool light = false;
        ...
        for(int test = 0; test< test_cfg::test_size; test++){
            for(int i = 0; i < 12; i++){
            ...
            test_cfg::confusion_matrix[static_cast<int>(test_cfg::data[test][0])][max]++;
        ...
        for(int digit = 0; digit < 10; digit++){
            ...
            std::cout << "Digit: accuracy: " << digit << " Total: " << 100.f * (float)test_cfg::confusion_matrix[digit][digit] / (float)sum << std::endl;
        }
        int corrects = 0;
        for (int i = 0; i < 10; i++){
        ...
        std::cout << "Total Accuracy: " << 100.f * (float)corrects / (float)test_cfg::test_size << std::endl;
